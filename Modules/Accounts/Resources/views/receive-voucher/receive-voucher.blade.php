    ...
    <div class="content-wrapper">
        <div v-if="!pageLoader">
            <section class="content-header">
                <h1>
                    <i class="fa fa-th-list"></i> Manage  |<small>Receive Voucher</small>
                </h1>
                <ul class="breadcrumb">
            ...
            <section class="content">
                ...
                <div class="box box-solid">
                    <div class="box-header with-border">
                        <h3 class="box-title"><i class="fa fa-search"></i> Receive Voucher List</h3>
                        <div class="box-tools">
                            <a class="btn btn-success btn-sm" @click="openModal('addForm', 'receive-voucher-data/create',true)"><i class="fa fa-plus-square"></i> New</a></div>
                    </div>
                    <div class="box-body">
                                    ...
                                    <template v-if="Object.keys(paginate_data).length > 0">
                                        <tr v-for="(list, index) in paginate_data" v-bind:key="index">
                                            ...
                                            <td>
                                                ...
                                                <a v-if="list.status==0" class="btn btn-primary btn-xs" @click="openModal('addForm', 'receive-voucher-data/'+list.id+'/edit')" title="Edit"><i class="fa fa-edit"></i></a>
                                                ...
                                                <a class="btn btn-primary btn-xs" @click="openModal('detailsForm', 'receive-voucher-data/'+list.id)" 
            ...
            <div class="modal-content" v-if="!pageLoader">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <span v-if="formData.id">Edit Receive Voucher</span>
                        <span v-else>Receive Voucher</span>
                    </h4>
                </div>
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <div class="form-group">
                                <label class="col-md-4 control-label required">Receive Date <span class="text-danger">*</span></label>
                    ...
                    <table class="responsive table table-striped table-bordered" cellspacing="0">
                        <thead>
                            <tr>
                                <th width="25%">Ledger (Cr)</th>
                                <th>Cr Amount</th>
                                <th width="25%">Receive By (Dr)</th>
                                <th>Dr Amount</th>
                                <th>Narration</th>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td valign="middle">
                                    <multiselect :select-label="''" :deselect-label="''" v-if="modalData.cashBankLedger" name="dr_sub_ledger_model" v-model="gridData.dr_sub_ledger_model" :options="modalData.cashBankLedger" placeholder="Select Ledger" label="accountCodeView" track-by="id" @input="selectDrLedger" :options-limit="10000"><span slot="noResult">Oops! No elements found. Consider changing the search query.</span></multiselect>
                                </td>          
                                <td valign="middle" class="text-right">
                    ...
                    <table class="responsive table table-striped table-bordered" cellspacing="0">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <label><input type="checkbox" true-value="1" false-value="0" v-model="gridData.dr_ac_payee" name="dr_ac_payee"> A/C Payee </label>
                                </td>
                                <td>
                                    <input type="text" name="dr_pay_to" v-model="gridData.dr_pay_to" class="form-control" placeholder="Pay to">
                                </td>
                                <td>
                                    <input type="number" name="dr_cheque_no" v-model="gridData.dr_cheque_no" class="form-control" placeholder="Cheque no">
                                </td>
                                <td>
                                    <v-date-picker v-model="gridData.dr_cheque_date" :config="dateOptions" style="width: 100%;" placeholder="Cheque date"></v-date-picker>
                                    
                                </td>
                                <td>
                                    <v-date-picker v-model="gridData.dr_draw_on" :config="dateOptions" style="width: 100%;" placeholder="Draw date"></v-date-picker>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            <template v-if="formData.itemAdded=='yes'">
                                <template v-for="(data, index) in formData.voucherDebitData" v-bind:key="index">
                                    <tr>
                                        <td valign="middle">@{{data.dr_accountCode}}
                                        <template v-if="data.pay_ledger=='bank'">
                                            <p v-if="data.dr_ac_payee==1" style="margin: 0;"><b>A/C Payee:</b> Yes</p>
                                            <p v-if="data.dr_pay_to && data.dr_pay_to!=''" style="margin: 0;"><b>Pay to:</b> @{{data.dr_pay_to}}</p>
                                            <p v-if="data.dr_cheque_no && data.dr_cheque_no!=''" style="margin: 0;"><b>Cheque No:</b> @{{data.dr_cheque_no}}</p>
                                            <p v-if="data.dr_cheque_date && data.dr_cheque_date!=''" style="margin: 0;"><b>Cheque Date:</b> @{{data.dr_cheque_date}}</p>
                                            <p v-if="data.dr_draw_on && data.dr_draw_on!=''" style="margin: 0;"><b>Draw On:</b> @{{data.dr_draw_on}}</p>
                                        </template>
                                        </td>          
...
<script type="text/javascript">
    window.dataUrl = 'receive-voucher-data';
    ...
    var app = new Vue({
      ...
      methods:{
        ...
        selectDrLedger(ledger){
            if(ledger && ledger.id>0){
                ...
                if(this.modalData.bankLedgerIds.includes(ledger.id)){
                    ...
                    this.$set(this.gridData, 'dr_cheque_date_show', this.modalData.current_date);
                    this.$set(this.gridData, 'dr_cheque_date', this.modalData.current_date);
                }else{
                    this.$set(this.gridData, 'pay_ledger', 'cash');
                }
            }else{
            ...
            this.$set(this.gridData, 'dr_ac_payee', '0');
        },
        voucherGridAdd(event){
            event.preventDefault();
            if(((this.gridData.dr_sub_ledger && parseFloat(this.gridData.dr_amount)>0) || (this.gridData.cr_sub_ledger && parseFloat(this.gridData.cr_amount)>0)) && (this.gridData.pay_ledger=='cash' || (this.gridData.pay_ledger=='bank' && this.gridData.dr_cheque_no && this.gridData.dr_cheque_date))){
        ...
        checkVoucherNo(event, type){
            ...
            const _this = this;
            if(type){
                var type_of_voucher = (type=='Bank')?3:4;
                var dataParams = {
