...
@section('content')
    <div id="app">
        <div class="content-wrapper">
            <div v-if="!pageLoader">
                <section class="content-header">
                    <h1>
                        <i class="fa fa-th-list"></i> Manage |<small>Contra Voucher</small>
                    </h1>
                    <ul class="breadcrumb">
                ...
                <section class="content">
                    ...
                    <div class="box box-solid">
                        <div class="box-header with-border">
                            <h3 class="box-title"><i class="fa fa-search"></i> Contra Voucher List</h3>
                            <div class="box-tools">
                                <a class="btn btn-success btn-sm"
                                    @click="openModal('addForm', 'contra-voucher-data/create',true)"><i
                        ...
                        <div class="box-body">
                            <form action="" class="form-inline">
                                <div class="row" style="margin-bottom: 10px">
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            ...
                                            <select name="listPerPage" class="form-control" v-model="listPerPage"
                                                @change="getResults(1)">
                                                <option v-for="size in pageSize" :value="size">@{{ size }}
                                                </option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-sm-3">
                                        <v-date-picker v-model="filter.from_date" :config="dateOptions" style="width: 100%;"
                                            placeholder="From date"></v-date-picker>

                                    </div>
                                    <div class="col-sm-3">
                                        <v-date-picker v-model="filter.to_date" :config="dateOptions" style="width: 100%;"
                                            placeholder="To date"></v-date-picker>
                                    </div>
                                    ...
                                    <div class="col-sm-2">
                                        <input type="text" name="search_key" placeholder="Search by keyword"
                                            class="form-control" v-model="filter.search_key" style="width: 100%;"
                                            autocomplete="off">
                                    </div>
                                </div>
                                ...
                                <div class="row" style="margin-bottom: 10px">
                                    ...
                                    <div class="col-sm-1">
                                        ...
                                        href="{{url('accounts/signatory-config-data',"contra")}}"
                                        data-target="#globalModal" data-toggle="modal"
                                        data-modal-size="modal-lg">signatory-config</a>
                                    </div>
                                    <div class="col-sm-1">
                                        <button class="btn btn-primary" @click="getResults(1)"><i
                                                class="fa fa-search"></i> Search</button>
                                    </div>
                                    <div class="col-sm-1" style="padding-left: 0">
                                        <button type="button" class="btn btn-secondary"><i class="fa fa-print"></i> Print
                            ...
                            <div class="table-responsive" style="max-height: 500px">
                                <table class="table table-striped table-bordered m-b-0">
                                    <thead>
                                        <tr>
                                            <th width="6%">#</th>
                                            <th class="sortable" v-bind:class="getSortingClass('voucher_no')"
                                                @click="sortingChanged('voucher_no')">Voucher #</th>
                                            <th class="sortable" v-bind:class="getSortingClass('amount')"
                                                @click="sortingChanged('amount')">Amount</th>
                                            <th class="sortable" v-bind:class="getSortingClass('trans_date')"
                                                @click="sortingChanged('trans_date')">Date</th>
                                            <th class="sortable"
                                                v-bind:class="getSortingClass('accounts_transaction.status')"
                                                @click="sortingChanged('accounts_transaction.status')">Status</th>
                                            <th>Approved By</th>
                                            <th>Remarks</th>
                                    ...
                                    <tbody>
                                        <template v-if="Object.keys(paginate_data).length > 0">
                                            <tr v-for="(list, index) in paginate_data" v-bind:key="index">
                                                ...
                                                <td>@{{ list.remarks }}</td>
                                                <td>
                                                    <a v-if="list.has_approval=='yes'" class="btn btn-primary btn-xs"
                                                        @click="voucherApproval('journal-voucher-approval',list.id)"><i
                                                            class="fa fa-check-square" aria-hidden="true"></i> Approved</a>
                                                    <a v-if="list.status==0" class="btn btn-primary btn-xs"
                                                        @click="openModal('addForm', 'contra-voucher-data/'+list.id+'/edit')"
                                                        title="Edit"><i class="fa fa-edit"></i></a>
                                                    <a v-if="list.status==0" @click="deleteItem(list.id)"
                                                        class="btn btn-danger btn-xs" data-placement="top"
                                                        data-content="delete" title="delete"><i
                                                            class="fa fa-trash-o"></i></a>
                                                    <a class="btn btn-primary btn-xs"
                                                        @click="openModal('detailsForm', 'contra-voucher-data/'+list.id)"><i
                                                            class="fa fa-info-circle" aria-hidden="true"></i> Details</a>
                                                </td>
                                            </tr>
                                        </template>
                                        <template v-else>
                            ...
                            <div class="row">
                                <div class="col-sm-12 pull-right">
                                    <pagination v-model="currentPageNo" :total-page="totalPage" @change="getResults"
                                        :max-size="10" />
                                </div>
                            </div>
        ...
        <div class="modal" id="addForm" tabindex="-1" role="dialog" aria-labelledby="esModalLabel"
            aria-hidden="true">
                    ...
                    <div class="modal-content" v-if="!pageLoader">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">
                                <span v-if="formData.id">Edit Contra Voucher</span>
                                <span v-else>New Contra Voucher</span>
                            </h4>
                        </div>
                        <div class="modal-body">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label class="col-md-4 control-label required">Voucher No <span
                                                class="text-danger">*</span></label>
                                        <div class="col-md-8 p-b-15">
                                            <input type="text" name="voucher_no" v-model="formData.voucher_no"
                                                class="form-control" required data-vv-as="voucher name"
                                                v-validate="'required'" :readonly="formData.auto_voucher"
                                                placeholder="Enter voucher no">
                                            <span class="error"
                                                v-if="$validator.errors.has('voucher_no')">@{{ $validator.errors.first('voucher_no') }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8 p-b-15">
                                            <input type="text" name="manual_ref_no" v-model="formData.manual_ref_no"
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <div class="form-group">
                                        <label class="col-md-4 control-label required">Contra Date <span
                                                class="text-danger">*</span></label>
                                        <div class="col-md-8 p-b-15">
                                            <v-date-picker name="trans_date" data-vv-as="Date" v-validate="'required'"
                                                v-model="formData.trans_date" :config="dateOptions" style="width: 100%;"
                                                placeholder="dd/mm/yyyy"></v-date-picker>
                                            <span class="error"
                                                v-if="$validator.errors.has('trans_date')">@{{ $validator.errors.first('trans_date') }}</span>
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <label class="col-md-4 control-label required">Narration <span
                                                class="text-danger">*</span></label>
                                        <div class="col-md-8">
                                            <textarea name="remarks" v-model="formData.remarks" placeholder="Narration" class="form-control" data-vv-as="Note"
                                                v-validate="'required'" maxlength="255"></textarea>
                                            <span class="error"
                                                v-if="$validator.errors.has('remarks')">@{{ $validator.errors.first('remarks') }}</span>
                                        </div>
                                    </div>
                            ...
                            <table class="responsive table table-striped table-bordered" cellspacing="0">
                                ...
                                <tbody>
                                    <tr>
                                        <td valign="middle" class="text-center" rowspan="2"
                                            style="vertical-align:middle;">
                                            <multiselect :select-label="''" :deselect-label="''" v-if="modalData.ledger"
                                                name="dr_sub_ledger_model" v-model="gridData.dr_sub_ledger_model"
                                                :options="modalData.ledger" placeholder="Select Ledger"
                                                label="accountCodeView" track-by="id" @input="selectDrLedger"
                                                :options-limit="10000"><span slot="noResult">Oops! No elements found.
                                                    Consider changing the search query.</span></multiselect>
                                        </td>
                                        <td valign="middle" class="text-right">
                                            <input type="number" name="dr_amount" v-model="gridData.dr_amount"
                                                class="form-control" placeholder="e.g.5000">
                                        </td>
                                        <td valign="middle">
                                            <multiselect :select-label="''" :deselect-label="''" v-if="modalData.ledger"
                                                name="cr_sub_ledger_model" v-model="gridData.cr_sub_ledger_model"
                                                :options="modalData.ledger" placeholder="Select Ledger"
                                                label="accountCodeView" track-by="id" @input="selectCrLedger"
                                                :options-limit="10000"><span slot="noResult">Oops! No elements found.
                                                    Consider changing the search query.</span></multiselect>
                                        </td>
                                        <td valign="middle" class="text-right">
                                            <input type="number" name="cr_amount" v-model="gridData.cr_amount"
                                                class="form-control" placeholder="e.g.5000">
                                        </td>
                                        <td class="text-right" valign="middle">
                                            <input type="text" name="narration" v-model="gridData.narration"
                                                class="form-control" placeholder="Remarks">
                                        </td>
                                        <td class="text-center" valign="middle" rowspan="2"
                                            style="vertical-align:middle;">
                                            <button type="button" class="btn btn-success"
                                                @click="voucherGridAdd($event)">Add</button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                                    ...
                                    <template v-if="formData.itemAdded=='yes'">
                                        <template v-for="(data, index) in formData.voucherDebitData" v-bind:key="index">
                                            <tr>
                                                ...
                                                <td class="text-center" valign="middle" style="vertical-align:middle;">
                                                    <button class="btn-xs btn-danger" title="Delete"
                                                        @click="voucherGridRemove($event, data, 'debit')"><i
                                        ...
                                        <template v-for="(data, index) in formData.voucherCreditData" v-bind:key="index">
                                            <tr>
                                                ...
                                                <td class="text-center" valign="middle" style="vertical-align:middle;">
                                                    <button class="btn-xs btn-danger" title="Delete"
                                                        @click="voucherGridRemove($event, data, 'credit')"><i
                                                            class="fa fa-trash"></i></button>
                                                </td>
                                            </tr>
                                        </template>
        ...
        <div class="modal" id="detailsForm" tabindex="-1" role="dialog" aria-labelledby="esModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-lg" style="width:1100px">
                <div class="modal-content" v-if="!pageLoader">
                    <div class="modal-header">
                    ...
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-success" @click="print('contra-voucher/'+formData.id)">
...
@section('scripts')
    <script type="text/javascript">
        window.dataUrl = 'contra-voucher-data';
        ...
        var app = new Vue({
            ...
            methods: {
                ...
                voucherGridAdd(event) {
                    ...
                    if ((this.gridData.dr_sub_ledger && parseFloat(this.gridData.dr_amount) > 0) || (this.gridData
                            .cr_sub_ledger && parseFloat(this.gridData.cr_amount) > 0)) {
                        if (this.gridData.dr_sub_ledger !== this.gridData.cr_sub_ledger) {
                            var checkExists = this.gridDataExist(this.gridData);
                            if (checkExists) {
                                alert("You have already added these Ledgers");
                                return false;
                            }
                    ...
                    if (type == 'debit') {
                        ...
                        if (data.dr_sub_ledger && data.cr_sub_ledger) {
                            this.formData.voucherCreditData.forEach(function(v, key) {
                                if (+data.dr_sub_ledger == +v.dr_sub_ledger && +data.cr_sub_ledger == +v
                                    .cr_sub_ledger) {
                                    v.dr_sub_ledger = 0;
                                    if (v.dr_amount) {
                                        v.dr_amount = 0;
                ...
                initNewVoucherData() {
                    const _this = this;
                    let URL = _this.baseUrl + '/contra-voucher-data/create';
