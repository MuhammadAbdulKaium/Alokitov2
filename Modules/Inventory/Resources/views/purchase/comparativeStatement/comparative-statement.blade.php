                        ...
                        <div class="box-header with-border">
                            ...
                            <div class="box-tools">
                                <a class="btn btn-success btn-sm"
                                    href="{{ url('inventory/comparative-statement-data/create') }}"><i
                        ...
                        <div class="box-body">
                            <form action="" class="form-inline">
                                <div class="row" style="margin-bottom: 10px">
                                    ...
                                    <div class="col-sm-3">
                                        <multiselect :select-label="''" :deselect-label="''" v-if="dataList.voucher_list"
                                            name="voucher_id_model" v-model="filter.voucher_id_model"
                                            :options="dataList.voucher_list" placeholder="Select voucher" label="voucher_no"
                                            track-by="id" @input="selectVoucher" :options-limit="10000"></multiselect>
                                    </div>
                                    <div class="col-sm-3">
                                        <multiselect :select-label="''" :deselect-label="''" v-if="dataList.item_list"
                                            name="item_id_model" v-model="filter.item_id_model"
                                            :options="dataList.item_list" placeholder="Select item" label="product_name"
                                            track-by="item_id" @input="selectFilterItem" :options-limit="10000">
                                        </multiselect>
                                    </div>
                                    <div class="col-sm-3">
                                ...
                                <table class="table table-striped table-bordered m-b-0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="sortable" v-bind:class="getSortingClass('item_id')"
                                                @click="sortingChanged('item_id')">Item Name</th>
                                            <th class="sortable" v-bind:class="getSortingClass('date')"
                                                @click="sortingChanged('date')">Date</th>
                                            <th class="sortable"
                                                v-bind:class="getSortingClass('inventory_comparative_statement_details_info.status')"
                                                @click="sortingChanged('inventory_comparative_statement_details_info.status')">
                                                Status</th>
                                            <th>Remarks</th>
                                            <th>Approved By</th>
                                        ...
                                        <template v-if="Object.keys(paginate_data).length > 0">
                                            <tr v-for="(list, index) in paginate_data" v-bind:key="index">
                                                <td><input type="checkbox" true-value="1" false-value="0"
                                                        v-model="list.del_id" @change="deleteCheck(list.del_id, list.id)">
                                                </td>
                                                ...
                                                <td>
                                                    <a v-if="list.has_approval=='yes'" class="btn btn-primary btn-xs"
                                                        @click="voucherApproval('comparative-statement-approval',list.id)"><i
                                                    ...
                                                    <a class="btn btn-primary btn-xs"
                                                        @click="openModal('detailsForm', 'comparative-statement-data/'+list.cs_id)"><i
                                                            class="fa fa-info-circle" aria-hidden="true"></i> Details</a>
                                                    <a class="btn btn-primary btn-xs"
                                                        :href="baseUrl+'/comparative-statement-history/'+list.cs_id"><i
                                                            class="fa fa-info-circle" aria-hidden="true"></i> Cs History</a>
                                                </td>
                                            </tr>
                                        </template>
                                        <template v-else>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-success" @click="PrintPurchase(formData.id)"> <i
