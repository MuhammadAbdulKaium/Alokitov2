...
class OnlineAcademicsController extends Controller
{
    ...
    public function StoreClassTopic(Request $request)
    {
        ...
        if ($request->class_status == 6) {

            $ScheduledData = $this->OnlineClassSchedule->where(['id' => $request->scheduleid])->update([
                'class_status' => 4,
                'class_topic_name' => $topiclist,
                'class_teacher_remarks' => $request->remark,
                'class_end_time' => date("Y-m-d H:i:s"),
                // 'class_duration_time'=>
            ]);

            return redirect('onlineacademics/onlineacademic/ClassHistory');
        } else if ($request->class_status == 2) {

            return redirect()->back()->with('success', 'You Do not Start The Class');
        }
    }
    ...
    public function ClassSchedule(Request $request)
    {
        ...
        $ClassScheduledId           = $institute_id . $campus_id . $academic_level_id . $academic_class_id . $academic_section_id . $academic_shift_id . $class_subject_id . $class_teacher_id . date("s");
