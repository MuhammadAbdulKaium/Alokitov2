					...
					@if($myPage=='result')
						...
						<div class="col-sm-6">
							<div class="form-group">
								<select id="promote_action" class="form-control col-sm-3 promote_action" name="promote_action" required>
				...
				<div class="col-sm-6">
					...
					<div class="btn-group pull-left" style="display:flex;">
						...
						<ul id="w5" class="dropdown-menu dropdown-menu-right">
							<li>
								<a class="download-passed-list" style="cursor:pointer">Download Approved List</a>
							</li>
						</ul>
					</div>
					@if($myPage=='result')
						<button  class="btn btn-primary pull-right submit_btn" type="submit">Select & Continue</button>
					@endif
				</div>
			</div>
		</div>
		...
		@if($myPage=='result')
			<div class="row">
				<div class="col-sm-12 box-footer">
					<button class="btn generate btn-primary pull-right submit_btn" type="submit">Select & Continue</button>
				</div>
			</div>
		@endif
	</form>
    ...
    $(document).ready(function () {
        ...
        $('.download-passed-list').click(function () {
            ...
            $('<form id="applicant_result_list_download_form" action="/admission/assessment/result/download" target="_blank" method="POST"></form>')
                ...
                .append('<input type="hidden" name="request_list_type" value="APPROVED"/>').appendTo('body').submit();
        ...
        $('form#applicant_promote_form').on('submit', function () {
            if(verifyStdList()){
                // promote_action
                var promote_action = $('#promote_action').val();
                // checking
                if(promote_action){
                    if(promote_action=='ADMIT'){
                        $(this).attr("action","/admission/assessment/admit/confirm");
                        $(this).attr("method","post");
                        $(this).append('<input type="hidden" name="academic_year" value="'+$("#academic_year").val()+'"/>')
                        $(this).append('<input type="hidden" name="academic_year_name" value="'+$("#academic_year option:selected").text()+'"/>')
                            .append('<input type="hidden" name="academic_level" value="'+$("#academic_level").val()+'"/>')
                            .append('<input type="hidden" name="academic_level_name" value="'+$("#academic_level option:selected").text()+'"/>')
                            .append('<input type="hidden" name="batch" value="'+$("#batch").val()+'"/>')
                            .append('<input type="hidden" name="batch_name" value="'+$("#batch option:selected").text()+'"/>');
