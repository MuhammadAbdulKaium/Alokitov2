...
<div id="applicant_result_container" class="box box-solid">
	...
	<div class="box-body" style="overflow-x:inherit">
        ...
        @if($examTaken==0)
            <p class="btn btn-default text-bold text-danger " style="color: red">
                <i class="fa fa-info-circle"></i> Exam has not published Yet. Go to Exam Setting for publishing Exam Result .
                Otherwise Student
                Can't
                Be Admitted.
            </p>
        ...
        <div class="row box-body" style="position: relative">
            @if($examTaken==1 || $examTaken==0)
                <ul id="applicant_result_list_btn_container" class="nav-tabs nav {{$applicantResultSheet->count()>0?'':'hide'}}">
                    ...
                    @if($examTaken==1)
                        <li class="list"><a id="ADMITTED" class="applicant-list">Admitted</a></li>@endif
                    `</ul>
...
<div id="applicant_result_list_container" class="col-md-12">
    @if($examSettingProfile)
        @if($examSettingProfile->exam_status && $examTaken==1 &&  $examSettingProfile->final_result_published==0 )
